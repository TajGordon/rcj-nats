...
class Piezo:
    ...
    FIXED VERSION with variable_frequency=True parameter.
    """
    ...
    def _init_pwm(self):
        """Initialize PWM for the piezo pin with variable_frequency=True."""
        try:
            # THE FIX: variable_frequency=True allows frequency changes!
            self.pwm = pwmio.PWMOut(self.pin, variable_frequency=True)
            print(f"✅ {self.name} PWM initialized with variable frequency")
        except Exception as e:
            print(f"❌ Error initializing PWM for {self.name}: {e}")
            self.pwm = None
    
    def play_note(self, frequency, duration=None, duty_cycle=32768):
        """
        ...
        Args:
            ...
            duty_cycle: PWM duty cycle (0-65535, default 32768 for 50%)
        """
        ...
        try:
            # Now we can change frequency directly!
    ...
    def play_note_async(self, frequency, duty_cycle=32768):
        """
        ...
        try:
            # Direct frequency change - much simpler now!
            self.pwm.frequency = int(frequency)
            self.pwm.duty_cycle = duty_cycle
            self.is_playing = True
        except Exception as e:
    ...
    def stop(self):
        ...
        if self.pwm is not None:
            try:
                ...
                print(f"❌ Error stopping note on {self.name}: {e}")
...
class Musician:
    ...
    FIXED VERSION that works with variable_frequency PWM.
    """
