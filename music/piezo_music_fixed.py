...
class Piezo:
    ...
    Fixed version that works with CircuitPython PWM limitations.
    """
    ...
    def _init_pwm(self):
        ...
        try:
            ...
            self.pwm = pwmio.PWMOut(self.pin, frequency=self.current_frequency, duty_cycle=0)
        except Exception as e:
            print(f"Error initializing PWM for {self.name}: {e}")
            self.pwm = None
    
    def _recreate_pwm(self, frequency):
        """Recreate PWM with new frequency (workaround for CircuitPython limitation)."""
        if self.pwm:
            self.pwm.deinit()
        ...
        try:
            ...
            print(f"Error recreating PWM for {self.name}: {e}")
            self.pwm = None
            return False
    ...
    def play_note(self, frequency, duration=None, duty_cycle=32767):
        ...
        # Recreate PWM if frequency changed
        if abs(frequency - self.current_frequency) > 1:
            if not self._recreate_pwm(frequency):
                return
    ...
    def play_note_async(self, frequency, duty_cycle=32767):
        ...
        # Recreate PWM if frequency changed significantly
        if abs(frequency - self.current_frequency) > 1:
            if not self._recreate_pwm(frequency):
                return
...
class Musician:
    ...
    def _process_line_events(self, line_idx, events):
        ...
        if self.current_position >= event_start:
            ...
            if frequency > 0:  # Only play if valid frequency
